...
Local single-user app (runs as a Uvicorn web server or as a packaged Windows desktop EXE) that translates Chinese light novels into English. Users paste text, upload `.txt` / `.docx` / `.epub` / `.html` files (single or bulk), or paste a public URL; the backend parses chapters, translates them one at a time using a user-selected AI provider, auto-builds a per-novel glossary so terminology stays consistent, and serves two surfaces: a READING reader (`/reader`: bilingual side-by-side with paragraph-ALIGNED rows in bilingual mode, TOC, bookmarks, quality badge, free-draft source toggle, inline glossary term marks (quiet dotted underlines, on by default, type-settings toggle, click opens the revise mini dialog), plus the reading-surface glossary flow restored 2026-08-06: select a phrase to Add/Revise a glossary entry via a mini dialog; NO text editing, no edit mode) and the CAT editor (`/editor`: the SINGLE text-editing surface: segment grid over the durable `chapter_segments` ledger, confirm tracking, TM/glossary assist rail, source-side glossary term marks, a para= position handoff so the reader and editor can hand off scroll position to each other, and the chapter tools util menu). Per-novel settings pick the translator provider, an optional refinement provider, and a genre (xianxia, wuxia, modern-romance, isekai, slice-of-life, mystery, litrpg, sci-fi, fantasy, yuri-bl). First-run launches a welcome wizard that walks the user through adding a provider before the rest of the UI unlocks.

**One LLM call per chapter from the selected provider.** No humanizer in the default flow; an opt-in per-novel refinement pass runs a second provider over the draft when configured. Each prompt carries the glossary, the previous-chapter tail, the user's editing history (the CAT ledger feeds all three arms: approved/confirmed zh->EN pairs, and USER STYLE PREFERENCES before-after pairs from edited segments merged ahead of the legacy `style_edits` rows), and the genre-aware system instruction composed from `backend/prompts/`. The translator owns every correctness axis AND the English prose itself.
...
│   │   ├── observations.py        # observer hits + POST .../observations/recheck (pull-based re-run for the editor QA panel)
...
│   │   ├── segments.py            # CAT editor: GET segments feed (lazy backfill on read) + PATCH {seg_index} / POST confirm-all writes (structured 409s: stale_segment/stale_chapter/chapter_translating) + GET {seg_index}/assist (TM exact/fuzzy + machine_text) + GET /novels/{id}/editor-next (continue card)
│   ├── services/
...
│   │   ├── segmentation.py        # 1:1 paragraph contract: pre-joined effective source paragraphs + target split; owns the frozen split/heading helpers (SEGMENTATION_VERSION; v3 also covers tm.py's retro-alignment cost model, so an aligner change bumps it too)
│   │   ├── segments.py            # CAT segment store (sole owner of chapter_segments): displayed_body rule, lazy build from the COMMITTED text, self-heal on read, update_segment/confirm_all state machine (body rematerialized from join(targets) same-transaction), human-row-preserving rebuilds, reproject_from_body hook for text-authoritative mutators; Phase 4: apply_machine_translation worker/refiner merge (human rows verbatim, machine_text refresh), prefill_confirmed_exact, approved_prompt_pairs, segment_assist; Phase 5: fetch_confirmed_exemplar_pairs (prompt exemplars) + recent_edited_pairs (style-pairs prompt arm), corpus_for_consistency / search_segments / concordance_search / chapter_ids_with_segments (provenance TM surfaces), next_chapter_to_edit (continue card)
...
│   │   ├── observations.py        # normalize observer outputs into NormalizedObservation rows + recheck_body_observations (scoped replace, dismissal carry-over)
...
│   │   ├── consistency.py         # deterministic fuzzy-TM + locked-term-missing detectors (read-only, on-demand; corpus prefers chapter_segments with status ranking, tm_segments legacy fallback); the narrowed glossary tier feeds the editor's Missing-locked-terms rail via GET /consistency, whose fuzzy tier the editor ignores (segment assist covers it)
...
│   └── js/
│       ├── api.js, theme.js, utils.js, spine.js, queue-panel.js, boot.js, command-palette.js  # shared; utils.js also carries the cross-tab bus (broadcastNovelChange/onNovelChange over BroadcastChannel "novel-changes", types appended/inserted/glossary) + glossaryApplyToastText
│       ├── term-marks.js      # shared, self-contained glossary term-mark matcher/wrapper for reader + editor (own esc(), no page globals, no DOM reads); loaded with defer on both pages before their page modules, deliberately outside the boot-safety concat contract on both pages (test_reader_js_boot_safety.py concatenates reader*.js and editor-*.js per page; term-marks.js matches neither prefix)
│       ├── reader-core.js, reader-toc.js, reader-chapter.js, reader-glossary.js, reader-quality.js  # reader split into ordered modules (plain <script> tags, shared scope; core first owns state + DOM handles and redirects ?mode=edit to /editor, chapter owns rendering/nav/bookmarks/boot + the aligned bilingual grid (client Gale-Church-lite _alignParas, restored 2026-08-06), glossary = the reading-surface selection popover + add/revise mini dialog (lean rebuild, no edit mode), quality last = the badge). Text editing stays editor-only: no contenteditable, no rails, no mode toggle.
...
2. Gathers prompt inputs: glossary, previous-chapter tail (within `PREVIOUS_CONTEXT_MAX_GAP` chapters back, last `PREVIOUS_CONTEXT_PARAGRAPHS` paragraphs bounded to `PREVIOUS_CONTEXT_MAX_CHARS`), captured style edits (USER STYLE PREFERENCES: the segment ledger's edited|confirmed `origin='human'` before-after pairs via `segments.recent_edited_pairs`, merged ahead of legacy `style_edits` rows under the one flag and cap; the chapter being translated is excluded on both arms, and pairs whose AFTER already rides the approved block are deduped at the fetch site on `pair_side_key`), per-novel style note, the confirmed-exemplar pairs (CAT Phase 5: recent confirmed segments from OTHER chapters, rendered as the APPROVED TRANSLATION EXAMPLES block next to the style-edits block), plus the resolved Provider and the novel's `genre` + `custom_style_brief` for the genre-aware system instruction. Author update-count / vote-begging markers (（第四更！）, 求月票) are stripped from the prompt's title line and the body's heading line (`parser.strip_title_update_marker`; prompt-time only, stored source stays verbatim).
3. Single LLM call via `translate_chapter` → backend's `_complete(prompt)` → `parse_delimited_response`. On parse failure: one retry, then plain-text fallback (sets `translation_degraded=1`, the only remaining degraded signal; the attempt row records `parse_error`, and the snapshot stamps `plain_fallback: true` with every block `*_included` key false, because the fallback prompt carries none of the blocks). The degenerate heading-only branch (no effective source paragraphs) runs with count validation off and therefore bypasses the llm_cache in both directions (`cacheable=False`).
...
5. **CAT worker merge (Phase 4)**: inside the claim-guarded success transaction (the claim UPDATE opens it, so human rows are re-read after the claim and editor writes cannot interleave), `segments.apply_machine_translation(kind='llm')` reconciles the fixed-up machine paragraphs with the `chapter_segments` store: human rows (edited|confirmed) keep `target_text` VERBATIM and only refresh `machine_text` with the new AI rendering; machine rows regenerate (origin `llm`); cross-chapter exact confirmed matches pre-fill as `origin='tm_exact'` (`prefill_confirmed_exact`: indexed hash lookup, full source-text equality against 16-hex collisions, most-recently-confirmed wins). The COMMITTED body is the merged join, so a retranslate can never clobber confirmed work. A storeless chapter gets a fresh build; on paragraph-count drift the machine side aligns via `tm.full_alignment_path` (merge/split-aware since 2026-08-06: 2:1/1:2 group moves with drops as a 4x last resort, the pre-pivot client aligner's cost model, so legacy pre-contract chapters pair sanely; a merge lands on the group's first row, follow-on row empty, both flagged; uncertain rows `aligned=0`, honored for human rows too: their `machine_text` refreshes only from confident slots and an unconfident slot demotes the chapter to `partial`); an unalignable merge RETAINS every row (I3) and stamps `unaligned`. A failed translation rolls the transaction back and never touches the store. **Origin is provenance** (bug-hunt Batch 2): a text-authoritative rewrite (reproject, rebuild) that CHANGES a human row's target stamps `origin='reprojected'` (status untouched); style-pair feeds require `origin='human'`, exemplars/endorsement feeds exclude `'reprojected'`; a per-row save/confirm restores `'human'`, bulk confirm-all never touches origin.
6. **Observations only** (`services/text_observers.py`; no retry, no degraded mark): `body_correctness_observations` runs `missing_translator_terms`, `detect_mt_texture`, `detect_double_possessive`, `detect_intensifier_inflation_on_glossary_term`, `detect_mid_sentence_paragraph_break`, `detect_glossary_predicate_loss`, computed on the FINAL merged body (observers see what the reader sees). Hits are logged at INFO. The single-pass thesis is that noticing has to happen inside the translator's thinking phase; a retry would be the same shallow pass twice. The same observer set also re-runs on demand via `POST .../observations/recheck` (editor QA panel: on open, on the Re-check button, debounced after segment saves), which scope-replaces only the body-correctness kinds (`observations.BODY_RECHECK_KINDS`), carries dismissals over on (kind, excerpt), and 409s while the chapter is translating.
...
**tm_segments is legacy-fallback for the TM read surfaces (CAT Phase 5).** The consistency rail corpus and the concordance search read `chapter_segments` first (status-carrying, rematerialized on every editor write) and fall back to `tm_segments` ONLY for chapters that have no segment rows; a segment-covered chapter's tm rows are suppressed even when its segment targets are empty. `tm_segments` is still written at every translate commit (alignment paths and `segment_assist`'s hash conventions also live in `tm.py`); removing the write is future work, not this phase. All chapter_segments SQL stays in `services/segments.py` (single-owner rule, binding).
...
- **Experiment instruments** (global env flags, flip via env for A/B; the style + refiner flags default `true`/parity, `PROMPT_INCLUDE_FREE_DRAFT` now defaults `false`):
  ...
  - `PROMPT_INCLUDE_REFINER` — global refiner kill-switch (overrides per-novel `refinement_provider_id` when false). Consulted on every entry path: fresh-translate queueing, the retry-refinement route (409), drain_on_startup's respawn (pending rows are HELD, never demoted, so flipping back on resumes them), and a belt no-op at the worker entry.
  ...
  - `PROMPT_INCLUDE_CONFIRMED_EXEMPLARS`: APPROVED TRANSLATION EXAMPLES block (CAT Phase 5: the 5 most recently confirmed segment pairs from OTHER chapters, deduped by source, 400 chars/side, voice + phrasing precedent). Defaults `true`. Distinct from APPROVED TRANSLATIONS (that block is same-chapter verbatim reuse; this one teaches voice cross-chapter); both can appear in one prompt. Absent when nothing is confirmed, so a no-exemplar prompt stays byte-identical to the pre-Phase-5 shape (no template-version bump); a present block folds into the llm_cache key by riding the prompt body. Confirm-as-is rows stay eligible (endorsement) but `origin='reprojected'` rows are excluded. Fetcher: `prompt_inputs.fetch_confirmed_exemplars` over `segments.fetch_confirmed_exemplar_pairs`.
...
- `services/find_replace.py::apply_in_place_for_glossary_term` rewrites a renamed `term_en` across existing translations in place: alias-aware (both sides slash-split; an old alias absent from the new set collapses onto the new primary via one longest-first alternation, so overlapping aliases like "Bai Xiaochun / Xiaochun" resolve correctly instead of corrupting), skips chapters mid-refinement and mid-translate with honest counts (the refiner's commit would clobber a rewrite on machine rows), and records an `fr_snapshots` restore point per touched novel (title_en included) so the apply is undoable from the Find/Replace History tab. Glossary changes broadcast to every open tab (reader, editor, glossary page) over the cross-tab bus. Unlike `retranslate-affected`, apply-in-place CAN fix segment rows a user already edited or confirmed in the CAT editor (origin stamped `reprojected`, status preserved); retranslate-affected keeps those rows verbatim through the worker merge and cannot touch them.
...
- **Novel-creation defaults**: `config_kv.novel_defaults` is a JSON blob (`{translator_provider_id, refinement_provider_id}`) written by the Settings page's Defaults section (restored 2026-08-10; no writer had ever existed before). `services/uploads.py::_resolve_novel_defaults` reads it inside the INSERT-novel path, so brand-new novels pick up the user's defaults instead of NULL. Genre and source_language were deliberately dropped from the whitelist on 2026-05-25 (genre is picked at import or set by the scrape recipe; source language is auto-detected) — do not re-add them to the Settings section without widening that whitelist first. Existing novels are never backfilled — the per-novel dialog on the library card remains the explicit override surface. Fields outside the whitelist are ignored, so a typo in the blob can't poison an unrelated column. Reserve `config_kv` for app-level state only; per-novel state belongs on the novels table.
...
- `pytest backend/tests`. Currently 1965 tests across 158 modules.
