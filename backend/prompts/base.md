...
- Restructure source-language comma-chains into natural English sentences, and restructure topic-comment fronting ("This thing, he had not considered") into English subject-verb-object order, unless the fronted topic is rhetorically loaded. Do NOT invent a causal or contrastive relationship the source lacks. You MAY use a discourse connector ("However", "Furthermore", "Thus", "Still", "So") to render cohesion the source achieves by other means, as long as the relationship it signals is one the source actually carries.
