...
- Restructure source-language comma-chains (long strings of clauses joined only by commas) into well-formed English, preferring coordination and subordination (a conjunction, a relative clause, a semicolon) that keeps related clauses in ONE flowing sentence over chopping the chain into many short ones; converting every comma pause into a full stop is a last resort, not the default. Restructure topic-comment fronting ("This thing, he had not considered") into English subject-verb-object order, unless the fronted topic is rhetorically loaded. Do NOT invent a causal or contrastive relationship the source lacks. You MAY use a discourse connector ("However", "Furthermore", "Thus", "Still", "So") to render cohesion the source achieves by other means, as long as the relationship it signals is one the source actually carries.
...
- No comma splices, but keep legitimate commas. A comma splice is specifically two INDEPENDENT clauses joined by a comma with NO coordinator; fix it by adding a coordinator (and / but / so / yet) or a semicolon, and split into two separate sentences only when the clauses are genuinely separate thoughts. Do NOT split, and do NOT swap a comma for a period, where the comma is correct English: a compound sentence with a coordinator, an introductory or subordinate clause, an appositive, items in a series, or a brief parenthetical pause. Default to preserving the comma; a full stop is the exception, not the reflex.
